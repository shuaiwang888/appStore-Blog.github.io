...
![1.png](/upload/2021/10/1-58148067f4714e6b86592d8f1febba23.png)
...
3. **ThreadLocalMap 中使用的 key 为 ThreadLocal 的弱引用,而 value 是强引用。所以，如果 ThreadLocal 没有被外部强引用的情况下，在垃圾回收的时候，key 会被清理掉，而 value 不会被清理掉。这样一来，ThreadLocalMap 中就会出现key为null的Entry。假如我们不做任何措施的话，value 永远无法被GC 回收，这个时候就可能会产生内存泄露。ThreadLocalMap实现中已经考虑了这种情况，在调用 set()、get()、remove() 方法的时候，会清理掉 key 为 null 的记录。使用完 ThreadLocal方法后 最好手动调用remove()方法**

4. 弱引用的目的是为了防止内存泄露，如果是强引用那么ThreadLocal对象除非线程结束否则始终无法被回收，弱引用则会在下一次GC的时候被回收。

5. 但是这样还是会存在内存泄露的问题，假如key和ThreadLocal对象被回收之后，entry中就存在key为null，但是value有值的entry对象，但是永远没办法被访问到，同样除非线程结束运行。

6. 但是只要ThreadLocal使用恰当，在使用完之后调用remove方法删除Entry对象，实际上是不会出现这个问题的。

![](https://img-blog.csdnimg.cn/8cb9f494e0014ea1a745c4dc823b58cf.png)
...
![2.png](/upload/2021/10/2-dfef75f5f7844a339e3c9cbc0b7c0f8b.png)
...
- **虚引用**：虚引用是最弱的引用关系，可以用PhantomReference来描述，他必须和ReferenceQueue一起使用，同样的当发生GC的时候，虚引用也会被回收。可以用虚引用来管理堆外内存。虚引用中唯一的作用就是用队列接收对象即将死亡的通知
...
![3.png](/upload/2021/10/3-82bffba213924d42bd29abc773a699d1.png)
...
![4.png](/upload/2021/10/4-2f6d816f1c9b4e9f9abe1e4a17be748c.png)
...
![5.png](/upload/2021/10/5-fa5e255390044335ae30c22dd0d8d792.png)

如果我们的**强引用**不存在的话，那么 `key` 就会被回收，也就是会出现我们 `value` 没被回收，`key` 被回收，导致 `value` 永远存在，出现内存泄漏。

### `ThreadLocal.set()`方法源码详解

![6.png](/upload/2021/10/6-3ead29ee1af44032953fdcb8512d3e5a.png)
...
![8.png](/upload/2021/10/8-92d6046f8f2e447f9859f644969e1457.png)
...
![7.png](/upload/2021/10/7-b43e50e51f294626be5535588f26d644.png)
...
![9.png](/upload/2021/10/9-111a1af39a9f4e13822739bd5b08d4dc.png)

这里直接将数据放到该槽位即可。

**第二种情况：** 槽位数据不为空，`key`值与当前`ThreadLocal`通过`hash`计算获取的`key`值一致：

![10.png](/upload/2021/10/10-45b8c2b018634ae68733099ad1344ffb.png)

这里直接更新该槽位的数据。

**第三种情况：** 槽位数据不为空，往后遍历过程中，在找到`Entry`为`null`的槽位之前，没有遇到`key`过期的`Entry`：

![11.png](/upload/2021/10/11-bf2ab2bc8cb64ca38fe0695601896b0f.png)
...
![12.png](/upload/2021/10/12-5dc559db5fd24da59c1edae87366affb.png)
...
![13.png](/upload/2021/10/13-4db072a05bbd4c26ac60c86a1e42d18c.png)
...
![14.png](/upload/2021/10/14-abb019152a9d4473a6980e35c602470b.png)
...
![view.png](/upload/2021/10/view-170f4e09268a41c6b13b24a9732165f7.png)

向后遍历过程中，如果没有找到相同 key 值的 Entry 数据：

![15.png](/upload/2021/10/15-13c1d1e79598411c9f9c7711e860a3fc.png)
...
![16.png](/upload/2021/10/16-9f7a4a2065694200b2d7883402360dcc.png)
...
![17.png](/upload/2021/10/17-d2b28ec6ff114cc5948f6a077dbb2637.png)
...
![18.png](/upload/2021/10/18-25d7b0f5fcef4299a15acba69b3b3242.png)
...
![19.png](/upload/2021/10/19-c9bcba06d3f944749a233a5487f7df2f.png)
...
![24.png](/upload/2021/10/24-f0ffd48e22794062b11c09729692c9f2.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![25.png](/upload/2021/10/25-67fbc76817e348fcb719b5dc07ed392b.png)
...
![26.png](/upload/2021/10/26-6edd6c54b2d94c7aa5b67aa2c3ca1148.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![27.png](/upload/2021/10/27-609485cd101548429452d2b828937e08.png)
...
![28.png](/upload/2021/10/28-f30163edfaf64c36b49e39d01d2ab884.png)
...
![29.png](/upload/2021/10/29-b6fc5a8527cc4e1e9ba66064aad9b62e.png)
...
![30.png](/upload/2021/10/30-2435861c4db84c5699fd4bd25104c606.png)

图中的`requestId`即为我们各个系统链路关联的`traceId`，系统间互相调用，通过这个`requestId`即可找到对应链路，这里还有会有一些其他场景：

![31.png](/upload/2021/10/31-a568311609324f259bf5a9adb016c48a.png)
