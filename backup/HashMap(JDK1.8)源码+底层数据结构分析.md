参考与：https://mp.weixin.qq.com/s/0Gf2DzuzgEx0i3mHVvhKNQ

> ### 问：hashMap中hash冲突后，新的Entry节点在插入链表的时候，是怎么插入的么？

**java8之前是头插法**，就是说新来的值会取代原有的值，原有的值就顺推到链表中去，因为写这个代码的作者认为**后来的值被查找的可能性更大一点，提升查找的效率。**
...
- Java8在同样的前提下并不会引起死循环，原因是扩容转移后前后链表顺序不变，保持之前节点的引用关系。或者说Java8时当链表长度大于8后，会将链表转成红黑树，尾插法可以更方便统计链表长度。

> ### 问：HashMap 为什么线程不安全？

**jdk1.7**

jdk1.7中的HashMap中扩容会造成死循环，扩容造成数据丢失。

从堆栈信息中可以看到出现死循环的位置，通过该信息可明确知道死循环发生在**HashMap的扩容函数**中，根源在transfer函数中，jdk1.7中HashMap的transfer函数如下：
...
HashMap中put操作的主函数中，如果没有hash碰撞则会直接插入元素。如果线程A和线程B同时进行put操作，刚好这两条不同的数据hash值一样，并且该位置数据为null，所以这线程A、B都会进入第6行代码中。假设一种情况，线程A进入后还未进行数据插入时挂起，而线程B正常执行，从而正常插入数据，然后线程A获取CPU时间片，此时线程A不用再进行hash判断了，问题出现：**线程A会把线程B插入的数据给覆盖，发生线程不安全。**
...
![](https://img-blog.csdnimg.cn/a6d12acb911d417b8c08b9069f2280bc.png)
...
![jdk1.8之前的内部结构.png](/upload/2021/10/jdk1.8%E4%B9%8B%E5%89%8D%E7%9A%84%E5%86%85%E9%83%A8%E7%BB%93%E6%9E%84-00cb65a395b44978a86d7856b5885922.png)
### JDK1.8 之后
...
![put方法.png](/upload/2021/10/put%E6%96%B9%E6%B3%95-0c49f1c55bf54ee487b8db657090f23b.png)
...
- 先生成一个新数组；
- 遍历老数组中的每个位置上的每个元素；
- 取每个元素的key，在新数组长度下进行计算下标，将其添加到新数组；
- 所有元素都转移完成后，将新数组赋值给hashMap对象的table属性；
