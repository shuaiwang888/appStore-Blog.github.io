以下参考：[限流算法](http://121.199.72.143:8090/archives/xian-liu-suan-fa--wei-wan-jie-)，本文讲述了固定窗口计数器算法、滑动窗口计数器算法、漏桶算法、 令牌桶算法（基于令牌桶实现的Guava中的RateLimiter，有平滑预热限流）、借助中间件架限流：Sentinel 或者使用 Redis 来自己实现对应的限流逻辑以及网关层限流（Spring Cloud Gateway 的分布式限流实现RedisRateLimiter就是基于 Redis+Lua 来实现的，再比如 Spring Cloud Gateway 还可以整合 Sentinel 来做限流。）
