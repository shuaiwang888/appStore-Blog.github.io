...
### 2.1 @EnableAutoConfiguration 是实现自动装配的重要注解。
...
### 2.2 AutoConfigurationImportSelector(加载自动装配类)
...
**注意：** 可以看出，AutoConfigurationImportSelector 类实现了 `ImportSelector` 接口，也就实现了这个接口中的 `selectImports` 方法，**该方法主要用于获取所有符合条件的类的全限定类名，这些类需要被加载到 IoC 容器中。**
...
### 2.3 现在我们结合getAutoConfigurationEntry()的源码来详细分析一下：
...
```
文件地址：spring-boot/spring-boot-project/spring-boot-autoconfigure/src/main/resources/META-INF/spring.factories
```
...
**第五步：** 最后新建工程引入 threadpool-spring-boot-starter
